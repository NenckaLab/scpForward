...
bool DcmStorageSCP::getStopRunning()
{
    return stopRunning;
}
...
// set methods
void DcmStorageSCP::setStopRunning(bool b)
{
    stopRunning = b;
}
    ...
    switch (DirectoryGeneration)
    {
        ...
        case DGM_AEName:
            ...
            if (dataset != NULL)
            {
                ...
                // alternatively, if that fails, use a default
                if (generatedDirName.empty())
                {
                    OFOStringStream stream;
                    stream << UndefinedSubdirectory << PATH_SEPARATOR
                    << "LostAndFound" << OFStringStream_ends;
                    OFSTRINGSTREAM_GETSTR(stream, tmpString)
                    generatedDirName = tmpString;
                    OFSTRINGSTREAM_FREESTR(tmpString);
                }
            } else {
                DCMNET_DEBUG("sending AETitle undetermined.");
                status = EC_CouldNotGenerateDirectoryName;
            }
            break;
    }
...
OFBool DcmStorageSCP::stopAfterCurrentAssociation()
{
    //printf("**** In derived stopAfterCurrentAssociation function. ****\n");
    return stopRunning;
}

// ----------------------------------------------------------------------------

OFBool DcmStorageSCP::stopAfterConnectionTimeout()
{
    //printf("**** In derived stopAfterConnectionTimeout function. ****\n");
    return stopRunning;
}
